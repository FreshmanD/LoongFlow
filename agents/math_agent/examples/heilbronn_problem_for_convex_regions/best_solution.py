...
def initial_placement(n):
    ...
    max_retries = 1000 # Limit retries to prevent infinite loops, though unlikely for n=13
    for _ in range(max_retries):
        points = random_initial_placement(n) # Always use random placement for robustness
    ...
    print(f"Warning: Failed to find non-degenerate initial placement after {max_retries} retries. Proceeding with potentially degenerate points.")
...
def simulate_heilbronn(n=13, num_runs=15):
    ...
    for run in range(num_runs):
        points = initial_placement(n) # Initialize points for the current run, ensuring non-degeneracy
        triangles, areas, point_triangles = precompute_triangles(points) # Precompute triangle data
        ...
        current_energy = -current_min_area # Energy is negative of min_area (maximize min_area -> minimize energy)
        ...
        total_move_attempts = 0         # Counter for total valid move attempts for adaptive step size
        ...
        for iter_count in range(MAX_ITER_PER_RUN):
            ...
            if current_min_area < 1e-9 or iter_count < MAX_ITER_PER_RUN / 10: 
                ...
                min_tri_indices = [i for i, area_val in enumerate(areas) if abs(area_val - current_min_area) < 1e-8]
            ...
            old_min_area_before_move = current_min_area # Store global min before this move attempt

            # Tentatively update the point and recompute areas of affected triangles
            points[idx] = new_point
            min_changed_area = float('inf') # Minimum area among triangles involving the moved point
    ...
    if len(best_overall_points) >= 3:
        try:
            ...
            if best_overall_min_area < MIN_VALID_TRIANGLE_AREA:
                convex_hull_area = 0.0 # If min_area is degenerate, so is the hull for ratio purpose
            else:
        ...
        except Exception: # Handle potential errors like all points collinear or too few points
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if points_are_close(points[i], points[j]):
                print(f"Verification FAILED: Points {points[i]} and {points[j]} are too close.")
                return False
    ...
    for i in range(n):
        for j in range(i + 1, n):
            for k in range(j + 1, n):
                ...
                if area < MIN_VALID_TRIANGLE_AREA:  
                    print(f"Verification FAILED: Degenerate triangle found with points {points[i]}, {points[j]}, {points[k]} (area: {area:.10f}).")
                    return False
                if area < min_computed:
                    min_computed = area
    
    # Check if the computed minimum area is close to the reported minimum area
    if abs(min_computed - min_area) < 5e-6: # Tolerance for reported vs. computed min_area
        return True
    else:
        print(f"Verification FAILED: Reported min area ({min_area:.10f}) does not match computed min area ({min_computed:.10f}).")
        return False
