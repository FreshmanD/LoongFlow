---
name: loongflow
description: "PEES (Plan-Execute-Evaluate-Summary) iterative problem-solving methodology with optional LoongFlow engine escalation. Use when tasks need structured iteration, optimization, evolution, or when user mentions loongflow/PEES/PES."
---
...
- Cons: Requires `ANTHROPIC_API_KEY` (only Anthropic models supported), setup time, runs as background process
- Source: https://github.com/baidu-baige/LoongFlow

**Ask the user which mode they prefer before proceeding.**

## Step 2a: Native PEES Mode

If the user chooses native mode, follow these instructions exactly.
...
## Step 2b: LoongFlow Engine Mode

If the user chooses engine mode, invoke the `loongflow-engine` skill. That skill handles all setup, configuration, launching, and monitoring.
